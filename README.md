...
При недоступности SAP API (ошибка 401 Unauthorized) система:

1. Пытается проверить соединение с API
2. При ошибке логирует проблему с подробностями
3. Автоматически переключается на использование тестовых данных
4. Продолжает функционировать, предоставляя тестовый датасет через API

Эта возможность позволяет продолжать разработку и тестирование даже при недоступности внешнего API.
